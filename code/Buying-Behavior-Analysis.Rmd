...
data_folder    = "/Users/nichada/MyCode/MPCS/_BUSN_DataSci"
proj_folder = "/DataSci_Mkt/data"
purchases_file = "purchases_beverages.RData"
products_file  = "products_beverages.RData"

load(paste0(data_folder, "/", proj_folder, "/", purchases_file))
load(paste0(data_folder, "/", proj_folder, "/", products_file))
```
...
```{r}
purchases = merge(purchases, products[, .(upc, upc_ver_uc, size1_units, size1_amount, multi)])
```
...
# Inspect the data
unique_values = unique(purchases$size1_units)
print(unique_values)
...
purchases[, volume := ifelse(size1_units == "OZ",
...
households_by_year = purchases[, .(no_households = length(unique(household_code))), by = year]
...
ggplot(purchases_category, aes(x = year, y = per_capita_volume, color = category)) +
  ...
  theme(
    ...
    axis.text.x = element_text(size = 6),    # X-axis tick mark text size
    axis.text.y = element_text(size = 6),    # Y-axis tick mark text size
    strip.text = element_text(size = 8)      # Facet label text size
  ) +
...
ggplot(purchases_category, aes(x = year, y = per_capita_spend, color = category)) +
  geom_line(size = 1) +
  labs(title = "Evolution of Per Capita Spend by Category",
       x = "Year",
       y = "Per Capita Spend (Dollars)") +
  theme_minimal() +
  facet_wrap(~ category, scales = "free_y") +  # Facet by category
  theme(
    plot.title = element_text(size = 10),    # Title text size
...
```{r}
# Normalize per capita purchase spend to 2004 values
purchases_category[, normalized_per_capita_spend := per_capita_spend / per_capita_spend[year == 2004], by = category]

# Plot the normalized per capita purchase spend
library(ggplot2)

ggplot(purchases_category, aes(x = year, y = normalized_per_capita_spend, color = category)) +
  geom_line(size = 1) +
  labs(title = "Evolution of Normalized Per Capita Spend by Category (2004 = 1.0)",
       x = "Year",
       y = "Normalized Per Capita Spend") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 10),    # Title text size
    axis.title.x = element_text(size = 8),   # X-axis label text size
...
```{r}
purchases = merge(purchases, products[.(upc,upc_ver_uc,brand_descr)], by = c("upc", "upc_ver_uc"))
```
